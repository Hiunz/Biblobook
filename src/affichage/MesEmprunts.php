...
<head>
  ...
  require_once "../classes/Exemplaire.php";
...
<body>
  <?php
    ...
    EmpruntController::getEmprunts($_SESSION['user'])
  ?>
                ...
                <table id="example" class="display nowrap dataTable dtr-inline collapsed" style="width: 100%;border: solid;margin-bottom: 10px;border-color: #b9b9b9;" aria-describedby="example_info">
                    <thead>
                        <tr>
                            ...
                            <th class="sorting" style="width: 70px; text-align: center;" aria-label="Date de l'emprunt">Dates de l'emprunt</th>
                    ...
                    <tbody>
                        <?php
                            $increment = 0;
                            foreach (EmpruntController::getEmprunts($_SESSION['user']) as $emprunt) {
                                $increment++;
                        ?>
                        <tr class="<?= ($increment%2 == 1)?"odd":"even" ?> parent">
                            <td class="sorting_1  dtr-control"><?= $emprunt->getLivre()->getTitre() ?></td>
                            <td><?= $emprunt->getLivre()->getAuteur()->getNom() ?></td>
                            <td><?= $emprunt->getLivre()->getCategorie() ?></td>
                            <td><?= date("d/m/y", strtotime($emprunt->getDateStart())).' - '.date("d/m/y", strtotime($emprunt->getDateStart())) ?></td>
                            <?php
                            if (!empty($_SESSION['user'])) {?>
                              <td class="selectRow">
                                  <form action="../traitement/GestionLivre.php" method="Post">
                                      <input type="hidden" name="livreSelect" value="<?= $emprunt->getLivre()->getId() ?>">
