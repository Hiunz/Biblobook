<?php
    class AuteurController{
        ...
        Public static function getAuteur($id_auteur){
            $bdd = (new Bdd())->getBdd();
        ...
        Public static function getAuteurs(){
            $bdd = (new Bdd())->getBdd();
            $req = $bdd->prepare('select auteur.*, pays.nom as pays_nom from auteur INNER JOIN pays on ref_pays = id_pays ;');
            $req->execute();
            $reps = $req->fetchAll();
            $auteurs = [];
            foreach ($reps as $rep) {
